...
def aggregate(results: list[dict[str, Any]]) -> dict[str, float | int]:
    return {
...
def checkpoint_corruption_probe() -> dict[str, str]:
    encoded = bytearray(encode_checkpoint({"sequence": 14, "tail_hash": "abc"}))
    encoded[len(encoded) // 2] ^= 1
    try:
        decode_checkpoint(bytes(encoded))
    except CheckpointError as exc:
        return {"status": "PASS", "observation": str(exc)}
    return {"status": "FAIL", "observation": "corrupted checkpoint was accepted"}
...
def validate_experimental_records() -> dict[str, Any]:
    try:
        ...
        return {
            "status": "UNKNOWN",
            "observation": "jsonschema is not installed",
        }
...
def run(mode: str) -> dict[str, Any]:
    ...
    starts_ratio = candidate["pump_starts"] / max(1, baseline["pump_starts"])
    energy_ratio = candidate["energy_kwh"] / max(0.000001, baseline["energy_kwh"])
    objective_pass = (
        starts_ratio <= 0.75
        and energy_ratio <= 1.10
        and candidate["unmet_demand_l"] <= baseline["unmet_demand_l"]
        and candidate["overflow_l"] <= baseline["overflow_l"]
    )
    scenario_gates_pass = all(
        not result["safety_violations"]
        and result["sequence_end"] == result["steps"]
        and (result["scenario_id"] != "checkpoint-restart" or result["restart_performed"])
    ...
    summary = {
        "schema_version": "0.1",
        "study_id": "mncs.remote-water-control.development-epoch-1",
        "mode": mode,
        "development_result": "PASS" if hard_gates_pass else "FAIL",
        "formal_mncs_status": "UNKNOWN",
        "formal_mncds_status": "UNKNOWN",
        "disposition": "REVIEW_REQUIRED",
        "claim_note": (
            "This development run does not claim MNCS-L5 or MNCDS-D3. Independent protected "
            "holdout evaluation, release binding, and operational evidence remain outstanding."
        ),
        "hard_gates": {
        ...
        "limitations": [
            "All scenarios are deterministic repository-visible development scenarios.",
            ...
            (
                "Cross-host, cross-architecture, protected-holdout, and independent "
    ...
    if mode == "all":
        ...
        (output / "scenario-results.json").write_text(
            json.dumps({"schema_version": "0.1", "results": observations}, indent=2) + "\n"
        )
