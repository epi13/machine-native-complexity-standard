...
def _d4_record() -> dict[str, object]:
...
def _d1_record() -> dict[str, object]:
    value = copy.deepcopy(_d4_record())
    value["profile"] = "MNCDS-D1"
    value["charter"]["planned_mncs_level"] = None  # type: ignore[index]
    value["partitions"]["holdout_id"] = None  # type: ignore[index]
    value["evaluators"] = [value["evaluators"][0]]  # type: ignore[index]
    value["candidates"][1]["evaluator_results"] = value["candidates"][1][  # type: ignore[index]
        "evaluator_results"
    ][:3]
...
def _d2_record() -> dict[str, object]:
    value = _d1_record()
...
def _d3_record() -> dict[str, object]:
    value = copy.deepcopy(_d4_record())
...
def test_generator_cannot_modify_evaluator_or_threshold() -> None:
    record = _d4_record()
    record["generator"]["permissions"]["modify_evaluators"] = True  # type: ignore[index]
    record["generator"]["permissions"]["modify_thresholds"] = True  # type: ignore[index]
    report = validate_development_value(record)
...
def test_candidate_lineage_cycle_is_rejected() -> None:
    record = _d4_record()
    record["candidates"][0]["parent_ids"] = ["candidate-b"]  # type: ignore[index]
    report = validate_development_value(record)
    assert not report.valid
...
def test_required_unknown_cannot_be_promoted_under_reject_policy() -> None:
    record = _d4_record()
    record["candidates"][1]["evaluator_results"][0]["status"] = "UNKNOWN"  # type: ignore[index]
    report = validate_development_value(record)
    assert not report.valid
...
def test_explicit_human_review_preserves_unknown_status() -> None:
    record = _d1_record()
    record["candidates"][1]["evaluator_results"][0]["status"] = "UNKNOWN"  # type: ignore[index]
    record["selection"]["unknown_policy"] = "human_review"  # type: ignore[index]
    record["selection"]["human_review"] = {  # type: ignore[index]
...
def test_d3_requires_independent_authority_and_executable() -> None:
    record = _d3_record()
    record["evaluators"][1]["authority_id"] = "authority-generation-team"  # type: ignore[index]
    record["evaluators"][1]["executable_id"] = "generator-runner-v2"  # type: ignore[index]
    report = validate_development_value(record)
    assert not report.valid
...
def test_mncs_binding_must_match_selected_candidate_and_charter() -> None:
    record = _d3_record()
    record["mncs_binding"]["candidate_id"] = "candidate-a"  # type: ignore[index]
    record["mncs_binding"]["environment_id"] = "other-environment"  # type: ignore[index]
    report = validate_development_value(record)
    assert not report.valid
...
def test_d4_requires_passing_rollback_and_regeneration_drill() -> None:
    record = _d4_record()
    record["release_controls"]["rollback_test_status"] = "UNKNOWN"  # type: ignore[index]
    record["release_controls"]["regeneration_drill"]["status"] = "FAIL"  # type: ignore[index]
    report = validate_development_value(record)
    assert not report.valid
    codes = {issue.code for issue in report.issues}
